...
namespace LisSoft.UI.Forms
{
    public partial class frm_AllMain : Form
    {
        /// <summary>
        ...
        private void BwPerformSearch(object sender, DoWorkEventArgs doWorkEventArgs)
        {
            ...
            {
                //Thread.Sleep(5000);
                Utilities.SetPropertyValue(progressBar, "Visible", true);
                Utilities.SetPropertyValue(progressBar, "Style", ProgressBarStyle.Marquee);
                ...
                if (ckbHasResult.Checked)
                {
                    ...
                    int sex = Utility.Int32Dbnull(Utilities.GetPropertyValue(cboSex, "SelectedValue"), -1);
                    int objectTypeId = Utility.Int32Dbnull(
                        Utilities.GetPropertyValue(cboObjectType, "SelectedValue"), -1);
                    int departmentId = Utility.Int32Dbnull(
                        Utilities.GetPropertyValue(cboDepartment, "SelectedValue"), -1);
                    grdPatients.SelectionChanged -= grdPatients_SelectionChanged;
                ...
                else
                {
                    ...
                    mv_DTPatientInfor = SPs.SpGetPatientInfoAll(dtpTestDateFrom.Value.ToShortDateString(),
                                                                ...
                                                                vBarcode, vPid, vName, vAge, sex, objectTypeId,
                                                                departmentId, vMaHis, vIsFinal, globalModule.gv_sUID)
                        .GetDataSet().Tables[0];
                    if (!mv_DTPatientInfor.Columns.Contains("CLSID"))
                        mv_DTPatientInfor.Columns.Add("CLSID", typeof (Int32));
                    foreach (DataRow row in mv_DTPatientInfor.Rows)
        ...
        private void LoadDevice()
        {
            try
            {
                ...
                DataBinding.BindDataCombox(cboDevice, dt, "Device_ID", "Device_Name", "---Tất cả---");
            }
            catch (Exception ex)
            {
                Utility.ShowMsg(ex.Message);
            ...
            try
            {
                ...
                dtTestType =
                    new Select(LUserTestType.Columns.UserName, TTestTypeList.Schema.Name + ".*").From(
                        ...
                        Where(LUserTestType.Columns.UserName).IsEqualTo(LabLink.globalModule.gv_sUID).
        ...
        {
            //    if (lablinkhelper.Utilities.gv_sUID == "ADMIN" || lablinkhelper.Utilities.gv_sUID == "LIS_ADMIN")
        ...
        private void cmdXoaCheck_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Utilities.gv_sUID == "ADMIN" || Utilities.gv_sUID == "LIS_ADMIN" || Utility.Int32Dbnull(grdPatients.GetValue("Print_Status")) == 0)
              {
                //    //Insert_Update_Result();
                ...
                if (!Utility.AcceptQuestion("Thực hiện xóa " + rowCollection.Length + " bản ghi", "Thông báo", true))
                  ...
                  foreach (GridEXRow grdRow in rowCollection)
                  {
                          ...
                          !string.IsNullOrEmpty(Utility.sDbnull(grdRow.Cells["Test_Result"].Value)))
                      {
                          ...
                          deleteLog.UserName = Utilities.gv_sUID;
        ...
        private void tsmDuyetChiDinh_Click(object sender, EventArgs e)
        {
            try
            {
                if (grdTestInfo.CurrentRow.Cells["SentStatus"].Value.ToString() == "1")
                {
                    ...
                    if (printStatus == "1")
                    {
                        if (LabLink.globalModule.gv_sUID == "ADMIN" || LabLink.globalModule.gv_sUID == "TRUC" ||
                            LabLink.globalModule.gv_sUID == "TRUONGKHOA" || LabLink.globalModule.gv_sUID == "PHOKHOA")
                        {
        ...
        private void tsmHuyDuyet_Click(object sender, EventArgs e)
        {
            ...
            //    if (printStatus == "1")
            //    {
            //        if (LabLink.globalModule.gv_sUID == "ADMIN" || LabLink.globalModule.gv_sUID == "TRUC" || LabLink.globalModule.gv_sUID =="TRUONGKHOA"|| LabLink.globalModule.gv_sUID =="PHOKHOA")
            //        {
            //            if (Utility.AcceptQuestion(
            //                string.Format("Cho phép xử lý kết quả xét nghiệm {0} của bệnh nhân {1}",
            //                              Utility.sDbnull(grdTestInfo.GetValue("TestType_Name")),
