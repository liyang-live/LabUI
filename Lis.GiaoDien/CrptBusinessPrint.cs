...
namespace VietBaIT.LABLink.Reports
{
    public class CrptBusinessPrint
    {
       ...
       // public string path = string.Format("{0}{1}", globalVariables.gv_PathReport_BHYT, "NoiTiet_A5_General_crpt_DetailTestReport_ALL_New.rpt");
        public DateTime NGAY = VNS.Libs.Utility.getSysDate();
        public VNS.Libs.frmPrintPreview _oForm;

        public void Print(bool Quick,string printerName)
        {
            string path = "";// = VietBaIT.CommonLibrary.SystemReports.GetPathReport(StrCode);
            try
            {
                ...
                if (ValidData())
                {
                    ...
                    if (SysPara.ParamaterReport != null)
                        ...
                        if (Quick)
                        {
                            ...
                            //  mv_oNguoiKy = new VietBaIT.CommonLibrary.cls_SignInfor(string.IsNullOrEmpty(oForm.crptTrinhKyName) , Crpt.ToString(),oForm.crptTrinhKyName, "", "");
                            mv_oNguoiKy = new cls_SignInfor_new(string.IsNullOrEmpty(oForm.crptTrinhKyName) ? oForm.crptTrinhKyName : oForm.crptTrinhKyName, "");
                            //      mv_oNguoiKy = new VietBaIT.CommonLibrary.cls_SignInfor(" ", "", Crpt, ReportSourceTable);
                    ...
                    else
                    {
                      ...
                      //  if (Quick) Crpt.PrintToPrinter(0, false, 0, 0);
                        ...
                        {
                          //  var oForm = new VietBaIT.CommonLibrary.frmPrintPreview("Phiếu KQ Xét nghiệm", Crpt, true, ReportSourceTable.Rows.Count <= 0 ? false : true);
                         //   oForm.crptTrinhKyName = Path.GetFileName(path);
        ...
        public static ReportDocument GetCrptFromAssembly(string ReportCode)
        {
            try
            {
                ...
                string path = "";// VietBaIT.CommonLibrary.SystemReports.GetPathReport(ReportCode);
                ...
                string sName = Utility.sDbnull(drvRow[SysReport.Columns.TenFileMacDinh]);
                crpt.Load(path);
                return crpt;
