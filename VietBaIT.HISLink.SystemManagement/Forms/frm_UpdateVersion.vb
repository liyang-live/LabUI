...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.GroupBox1.Size = New System.Drawing.Size(402, 231)
        ...
        Me.CboVerType.Location = New System.Drawing.Point(108, 78)
        Me.CboVerType.Name = "CboVerType"
        Me.CboVerType.Size = New System.Drawing.Size(204, 24)
        Me.CboVerType.TabIndex = 2
        '
        'txtVersion
        '
        ...
        Me.txtVersion.Location = New System.Drawing.Point(108, 105)
        ...
        Me.txtDesc.Location = New System.Drawing.Point(108, 159)
        ...
        Me.txtCapacity.Location = New System.Drawing.Point(108, 51)
        ...
        Me.cmdClose.Location = New System.Drawing.Point(321, 240)
        ...
        Me.cmdSave.Location = New System.Drawing.Point(231, 240)
        ...
        Me.lblStatus.Location = New System.Drawing.Point(6, 237)
...
#End Region
        ...
        Try
            ...
            If mv_intStatus = 1 Then
                ...
                For Each sfile As String In lstfiles
                    ...
                    fs.Flush()
                    fs.Close()
                    If SaveFile(arrFilename(0), IIf(arrFilename(0).LastIndexOf(".") <> -1, arrFilename(0).Replace(arrFilename(0).Substring(arrFilename(0).LastIndexOf(".") + 1), "rar"), arrFilename(0) & ".rar"), arrData, txtVersion.Text.Trim, intRar, IIf(CboVerType.SelectedIndex = 0, 1, 0), CDec(txtCapacity.Text.Trim.ToUpper.Replace("BYTES", "")), txtDesc.Text.Trim) Then
            ...
            Else 'CẬP NHẬT FILE VERSION
                If mv_bFileHasChanged Then
                    ...
                    fs.Flush()
                    fs.Close()
                    If UpdateVersion(mv_intID, arrFilename(0), IIf(arrFilename(0).LastIndexOf(".") <> -1, arrFilename(0).Replace(arrFilename(0).Substring(arrFilename(0).LastIndexOf(".") + 1), "rar"), arrFilename(0) & ".rar"), arrData, txtVersion.Text.Trim, intRar, IIf(CboVerType.SelectedIndex = 0, 1, 0), CDec(txtCapacity.Text.Trim.ToUpper.Replace("BYTES", "")), txtDesc.Text.Trim, mv_bFileHasChanged) Then
                        lblStatus.Text = ""
                        lblStatus.Refresh()
                        If File.Exists(sRarFile) Then
                            File.Delete(sRarFile)
                ...
                Else 'Không thay đổi dữ liệu phiên bản
                    Dim arrData() As Byte
                    If UpdateVersion(mv_intID, "OK", "OK", arrData, "OK", 1, IIf(CboVerType.SelectedIndex = 0, 1, 0), 1000, txtDesc.Text.Trim, mv_bFileHasChanged) Then
                        lblStatus.Text = ""
                        lblStatus.Refresh()
                        '-------------------------------------------
                        For Each dr As DataRow In frm_ListUpdateVersion.mv_DSVersion.Tables(0).Rows
    ...
    Private Function SaveFile(ByVal pv_sFileName As String, ByVal pv_sRarFileName As String, ByVal pv_arrData() As Byte, ByVal pv_sVersion As String, ByVal pv_intRar As Int16, _
        ...
        Try
            ...
            lblStatus.Refresh()
            Me.Refresh()
            Dim strSQL As String = "INSERT INTO Sys_VERSION(sFileName,sRarFileName,objData,sVersion,intRar,intPatch,dblCapacity,sDesc,tUpdatedDate) VALUES(@_sFileName,@_sRarFileName,@_objData,@_sVersion,@_intRar,@_intPatch,@_dblCapacity,@_sDesc,@tUpdatedDate)"
    ...
    End Function
    Private Function UpdateVersion(ByVal pv_intID As Integer, ByVal pv_sFileName As String, ByVal pv_sRarFileName As String, ByVal pv_arrData() As Byte, ByVal pv_sVersion As String, ByVal pv_intRar As Int16, _
                        ByVal pv_intPatch As Int16, ByVal pv_decCapacity As Decimal, ByVal pv_sDesc As String, Optional ByVal pv_bFilehasChanged As Boolean = False) As Boolean
        ...
        Try
            ...
            If pv_bFilehasChanged Then 'Cập nhật có thay đổi dữ liệu File
                Dim strSQL As String = "UPDATE Sys_VERSION SET sFileName=@_sFileName,sRarFileName=@_sRarFileName,objData=@_objData,sVersion=@_sVersion,intRar=@_intRar,intPatch=@_intpatch,dblCapacity=@_dblCapacity,sDesc=@_sDesc,tUpdatedDate=@_tUpdatedDate WHERE PK_intID=" & pv_intID
                Dim cmd As New SqlCommand(strSQL, VNS.Libs.globalVariables.SqlConn)
                With cmd
                    .Parameters.Add(New SqlParameter("@_sFileName", SqlDbType.NVarChar)).Value = pv_sFileName
                    .Parameters.Add(New SqlParameter("@_sRarFileName", SqlDbType.NVarChar)).Value = pv_sRarFileName
            ...
            Else 'Cập nhật các thông tin khác ngoài dữ liệu File
                Dim strSQL As String = "UPDATE Sys_VERSION SET intPatch=@_intpatch,sDesc=@_sDesc,tUpdatedDate=@tUpdatedDate WHERE PK_intID=" & pv_intID
                Dim cmd As New SqlCommand(strSQL, VNS.Libs.globalVariables.SqlConn)
                With cmd
                    .Parameters.Add(New SqlParameter("@_intPatch", SqlDbType.SmallInt)).Value = pv_intPatch
                    .Parameters.Add(New SqlParameter("@_sDesc", SqlDbType.NVarChar)).Value = pv_sDesc
