    ...
    public class AutoCompleteTextbox_Nhanvien : TextBox
    {
        ...
        public void Init()
        {
            try
            {
             ...
             var query = from p in m_dtDanhmucChung.AsEnumerable()
                         select "-1#" + p.Field<string>(VDmucNhanvien.Columns.MaNhanvien).ToString() + "@" + p.Field<string>(VDmucNhanvien.Columns.TenNhanvien).ToString() + "@" + p.Field<string>("shortcut").ToString();
        ...
        public void Init(DataTable dtData)
        {
            try
            {
                ...
                DataRow[] arrDr = dtData.Select("Ma_loainhanvien='" + LOAI_NHANVIEN + "'", VDmucNhanvien.Columns.SttHthi);
                if (arrDr.Length <= 0)
                    return;
